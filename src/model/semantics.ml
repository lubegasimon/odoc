...
let rec nestable_block_element :
 ...
 fun status element ->
  ...
  | { value = `Paragraph (p_style, content); location } ->
      Location.at location
        (`Paragraph (p_style, inline_elements status content))
...
let section_heading :
 ...
 fun status ~top_heading_level location heading ->
  let (`Heading (level, label, content (* , _ *)
                                       (*@ heading*))) = heading in

  (*TODO: Hmm, not sure if paragraph style should be ignored!*)
